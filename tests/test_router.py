...
from cobaiter.schemas import ChatCompletionRequest, Route
...
async def test_different_conversations_can_differ(engine, classifier):
    classifier.table = {"claude-opus-4-8": 0.9, "qwen2.5": 0.95}
    a = await engine.decide(user_req("hello", metadata={"tier": "rich"}), header_id="a")
...
async def test_tier_hint_restricts_candidates(engine, classifier):
    classifier.table = {"qwen2.5": 0.99, "claude-haiku-4-5": 0.2}
    d = await engine.decide(user_req("simple", metadata={"tier": "openweight"}), header_id="c1")
    assert d.model == "qwen2.5"
